...
def seed_events():
    ...
    with app.app_context():
            ...
            for org_type, orgs in ORGANIZATIONS.items():
                ...
                for i in range(2):
                    ...
                    event_date = event_date.replace(
                        hour=random.choice([9, 10, 13, 14, 15, 18, 19]),
                        minute=0,
                        second=0,
                    )
                    ...
                    event = Event(
                        ...
                        description=template["description"].format(
                            org_name=org["name"], city=city_name
                        ),
                    ...
                    event_image = EventImage(
                        event_id=event.id, url=image_url, position=0
                    )
        ...
        print(
            f"   Organizations: {sum(len(orgs) for orgs in ORGANIZATIONS.values())} total"
        )
