...
class TestVolunteerCompleteJourney:
    ...
    def test_volunteer_full_workflow(self, client):
        ...
        login_response = client.post(
            "/login",
            json={"email": "volunteer@example.com", "password": "secure_password123"},
        )
        ...
        assert (
            view_listing_response.json["title"] == "Community Garden Volunteers Needed"
        )
...
class TestOrganizationCompleteJourney:
    ...
    def test_organization_full_workflow(self, client):
        ...
        # Step 2: Login
        login_response = client.post(
            "/login", json={"email": "org@nonprofit.org", "password": "org_password123"}
        )
        assert login_response.status_code == 200
        access_token = login_response.json["access_token"]
        headers = {"Authorization": f"Bearer {access_token}"}
...
class TestVolunteerSignupWorkflow:
    ...
    def test_volunteer_signup_flow(self, client):
        ...
        signups_response = client.get(
            f"/listings/{listing_id}/signups", headers=org_headers
        )
        ...
        accept_response = client.put(
            f"/signups/{signup_id}", headers=org_headers, json={"status": "accepted"}
        )
    ...
    def test_review_submission_flow(self, client):
        ...
        for i in range(3):
            ...
            review_response = client.post(
                ...
                headers=headers,
                json={"rating": rating, "comment": f"Great experience! Review #{i+1}"},
            )
...
class TestEventDiscoveryWorkflow:
    ...
    def test_event_discovery_and_personalization(self, client):
        ...
        surprise_response = client.post(
            "/api/events/surprise-me",
            headers=headers,
            json={"location": "Austin, TX", "mood": "creative", "budget": 50},
        )
...
class TestPasswordResetWorkflow:
    ...
    def test_password_reset_integration(self, client):
        ...
        client.post(
            "/register",
            json={"email": email, "password": original_password, "role": "volunteer"},
        )

        # Step 2: Login with original password
        login1 = client.post(
            "/login", json={"email": email, "password": original_password}
        )
        ...
        if "reset_url" in reset_request.json:
            token = reset_request.json["reset_url"].split("/reset-password/confirm/")[
                -1
            ]
            confirm = client.post(
