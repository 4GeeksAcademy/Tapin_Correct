...
class TestEventInteraction:
    ...
    def test_record_interaction_missing_fields(self, client, auth_headers):
        ...
        response = client.post(
            "/api/events/interact", headers=auth_headers, json={"event_id": 1}
        )
        assert response.status_code == 400
        assert "error" in response.json
    ...
    def test_record_interaction_requires_auth(self, client, sample_events):
        ...
        response = client.post(
            "/api/events/interact",
            json={"event_id": sample_events[0], "interaction_type": "view"},
        )
    ...
    def test_record_multiple_interactions(self, client, auth_headers, sample_events):
        ...
        for interaction in interactions:
            response = client.post(
                "/api/events/interact", headers=auth_headers, json=interaction
            )
            assert response.status_code == 201
...
class TestPersonalizedEvents:
    ...
    def test_personalized_events_missing_location(self, client, auth_headers):
        ...
        response = client.post(
            "/api/events/personalized", headers=auth_headers, json={}
        )
        assert response.status_code == 400
        assert "error" in response.json
    ...
    def test_personalized_events_requires_auth(self, client):
        ...
        response = client.post(
            "/api/events/personalized", json={"location": "Austin, TX"}
        )
        assert response.status_code == 401
...
class TestTasteProfile:
    ...
    def test_get_taste_profile_with_interactions(
        self, client, auth_headers, sample_events
    ):
        ...
        for interaction in interactions:
            client.post("/api/events/interact", headers=auth_headers, json=interaction)
        ...
        assert (
            "category_preferences" in profile
            or "categories" in profile
...
class TestSurpriseMe:
    ...
    def test_surprise_me_missing_location(self, client, auth_headers):
        ...
        response = client.post(
            "/api/events/surprise-me", headers=auth_headers, json={"mood": "chill"}
        )
        assert response.status_code == 400
        assert "error" in response.json
    ...
    def test_surprise_me_requires_auth(self, client):
        ...
        response = client.post(
            "/api/events/surprise-me", json={"location": "Austin, TX"}
        )
        assert response.status_code == 401
