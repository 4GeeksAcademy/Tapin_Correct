"""Tests for listing CRUD operations and ownership verification."""
...
def test_create_and_crud_listing(client, create_user):
    ...
    resp = client.post(
        "/listings", json={"title": "L1", "description": "D1"}, headers=headers
    )
...
def test_delete_listing_ownership_verification(client, create_user):
    ...
    resp_other = client.delete(
        f"/listings/{listing_id}", headers={"Authorization": f"Bearer {other_token}"}
    )
    assert resp_other.status_code == 403
    assert "not the owner" in resp_other.get_json()["error"].lower()
    ...
    resp_owner = client.delete(
        f"/listings/{listing_id}", headers={"Authorization": f"Bearer {owner_token}"}
    )
    assert resp_owner.status_code == 200
