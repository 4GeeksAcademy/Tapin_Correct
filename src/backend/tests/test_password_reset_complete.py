...
class TestPasswordResetConfirm:
    ...
    def test_reset_confirm_valid_token(self, client, registered_user):
        ...
        reset_response = client.post('/reset-password', json={
            'email': registered_user['email']
        })
        ...
        if 'reset_url' in reset_data:
            ...
            confirm_response = client.post(f'/reset-password/confirm/{token}', json={
            ...
            old_login_response = client.post('/login', json={
                'email': registered_user['email'],
    ...
    def test_reset_confirm_invalid_token(self, client):
        ...
        response = client.post('/reset-password/confirm/invalid_token_xyz', json={
    ...
    def test_reset_confirm_token_reuse(self, client, registered_user):
        ...
        if 'reset_url' in reset_data:
            ...
            # First use
            first_response = client.post(f'/reset-password/confirm/{token}', json={
                'password': 'password_one'
            })
            assert first_response.status_code == 200

            # Second use (should still work if token not expired)
            second_response = client.post(f'/reset-password/confirm/{token}', json={
...
class TestPasswordResetSecurity:
    ...
    def test_reset_doesnt_reveal_user_existence(self, client, registered_user):
        ...
        existing_response = client.post('/reset-password', json={
            'email': registered_user['email']
        })

        # Request for non-existing user
        nonexistent_response = client.post('/reset-password', json={
            'email': 'nonexistent@test.com'
        })
...
class TestPasswordResetIntegration:
    ...
    def test_complete_password_reset_flow(self, client, registered_user):
        ...
        reset_request = client.post('/reset-password', json={
            'email': registered_user['email']
        })
    ...
    def test_multiple_reset_requests(self, client, registered_user):
        ...
        response3 = client.post('/reset-password', json={
            'email': registered_user['email']
        })
