...
test.describe("Volunteer Listings", () => {
  test("should display listings or empty state", async ({ page }) => {
    ...
    const hasListings =
      (await page
        .locator('[class*="listing"], [class*="card"], article')
        .count()) > 0;
  ...
  test("should have filterable listings by category", async ({ page }) => {
    ...
    // Use simpler selectors to detect filter controls without regex in :has-text
    const filterElements = await page
      .locator('select, [role="combobox"], button, [class*="filter"]')
      .count();
  });
  ...
  test("should have professional listing card styling", async ({ page }) => {
    ...
    const listingCard = page
      .locator('[class*="listing"], [class*="card"]')
      .first();

    if (await listingCard.isVisible()) {
      const borderRadius = await listingCard.evaluate(
        (el) => window.getComputedStyle(el).borderRadius
      );
      expect(borderRadius).not.toBe("0px");

      const boxShadow = await listingCard.evaluate(
        (el) => window.getComputedStyle(el).boxShadow
      );
      const border = await listingCard.evaluate(
        (el) => window.getComputedStyle(el).border
      );

      expect(boxShadow !== "none" || border !== "0px none rgb(0, 0, 0)").toBe(
        true
      );
    }
  });

  test("should show listing metadata (date, location, category)", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
